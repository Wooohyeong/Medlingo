import type { FullBackup, Question } from './types.js';
  ...
  if (name.endsWith('.csv')) {
    ...
    return rows.filter(Boolean).map((r, i) => {
      ...
      return norm({
        ...
        type: 'single',
        subject: c[idx('subject')] || '기타',
        system: c[idx('system')] || '일반',
        topic: c[idx('topic')] || '기본',
        ...
        answer: c[idx('answer')] || 'A',
        explanation: c[idx('explanation')] || '',
        createdBy: 'custom',
